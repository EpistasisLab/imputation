...
# load 1 10k example
by_var <- read.csv('~/data/imputation/imputation_scores/match_by_var.csv')
...
plot = ggplot(by_var_subset, aes(x=Method, y=MSE, color=Loinc_Codes)) + 
  ...
  theme_bw() +
  scale_fill_discrete(name="Lab Test (by LOINC code)") +
  ylab("RMSE")+
  scale_x_discrete(labels=labels) +
  theme(axis.text.x = element_text(angle=45, hjust = 1))

png("../figures/figure_4.png", width = 8, height = 5, res = 300, units = "in")
print(grid.arrange(top="Missing based on real data observations.",
                   arrangeGrob(plot), nrow=1, ncol=1))
dev.off()

# supplement 22
match_scores <- read.csv('~/data/imputation/imputation_scores/match.csv')
...
png("../figures/Supplemental_Figure_22.png", width = 8, height = 5, res = 300, units = "in")
print(grid.arrange(top="Missing based on real data observations.",
                   arrangeGrob(supp_plot), nrow=1, ncol=1))
dev.off()
