...
load("/data/trimmedMedianLabResultsPlusDemographics.Rfile")
...
# all panels missingness prediction
...
# top 28 missingness prediction
top_28_loinc <- c("718-7", "4544-3", "787-2", "786-4", "785-6", "6690-2", "789-8", "788-0", "32623-1", "777-3",
...
### Print six panels into one png
png("./figures/figure_2.png", width = 12, height=12, res = 300, units = "in")
...
### Supplement 1
pA<-ggplot(subset(top28, SEX %in% c("Male", "Female")), aes(x=medAge, y=missing.lipids, color=SEX, fill=SEX))+
  #  geom_hex()+
  xlab("Patient Age")+
...
pB<-ggplot(subset(top28, SEX %in% c("Male", "Female")), aes(x=BMI, y=missing.lipids, color=SEX, fill=SEX))+
  #  geom_hex()+
  xlab("Patient BMI")+
  ylab("Missing Lipid Results")+
  ggtitle("B.")+
...
pC = ggplot(scores_df, aes(x=reorder(Labs, -ROC_AUC), y=ROC_AUC, color=Labs)) +
  ggtitle("C.") +
  labs(y="ROC AUC", x="Labs") +
  geom_boxplot(outlier.colour="black", outlier.shape=1, outlier.size=1) + 
  theme_bw() +
...
top_28_loinc <- c("718-7", "4544-3", "787-2", "786-4", "785-6", "6690-2", "789-8", "788-0", "32623-1", "777-3",
                  ...
                  "30239-8", "1975-2", "2885-2", "10466-1", "751-8", "2093-3", "2571-8", "2085-9", "13457-7",
                  "2965-2")
select_scores<-scores[, top_28_loinc]
print(dim(select_scores))

colnames(select_scores) = top_28_loinc
...
pD = ggplot(select_scores_df, aes(x=reorder(Lab, -ROC_AUC), y=ROC_AUC, color=Lab)) +
  geom_boxplot(outlier.colour="black", outlier.shape=1, outlier.size=1) + 
  ggtitle("D.") +
  labs(y="ROC AUC", x="Labs") +
  theme_bw() +
...
png("./figures/Supplemental_Figure_1.png", width = 12, height=12, res = 300, units = "in")
grid.arrange(pA, pB, pC, pD, ncol = 2)
dev.off()

### make heatmap of correlations in labs data
load("/data/trimmedMedianTop28LabResultsPlusDemographics.Rfile")
cormat<-cor(ptTop28TrimMedResDemWide[,-c(1,2)], use = "pair")
colorRampPalette(c("white","aliceblue","blue","darkblue","black"))

png("./figures/Supplemental_Figure_2.png", width = 24, height=16, units = "in", res = 300)
